...
class commons {
    ...
    public static function users_category_detail($f3){
        ...
        $db = new \helper\database($f3, 'commonswiki', 'commonswiki');
        $res = $db->exec('select img_user_text user, img_name, img_size, img_metadata from page, image, categorylinks where page_title = img_name and cl_from = page_id and cl_to = ? and img_user_text = ifnull(?, img_user_text) and page_namespace = 6 order by 2 DESC;', array(1=>$cat_search, 2=>$user));
